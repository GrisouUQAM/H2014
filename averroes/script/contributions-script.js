...
function callback_Q6(response) {
    var avantCB = 0, apresCB = 0;
    var isPassed = false;
    var users;
    var article;
    var overflow = " ";
    var data = response.query.pages;
    var dateContributionUser,dateLastContribution,dateFirstContribution;

    //the key is a number which cannot be accessed with "."
    for(var key in data){
        users = data[key].revisions;
        article = data[key].title;
    }
    
    dateLastContribution = users[0].timestamp; 
    if(users.length === 500){overflow = " (over) " ;}
    for(var i = 0; i < users.length ; i++){
        if( (user.toLowerCase()).localeCompare( (users[i].user).toLowerCase() ) === 0 && !isPassed){
            isPassed = true;
            dateContributionUser = users[i].timestamp;
        }else if(isPassed){
            avantCB++;
        }else if(!isPassed){
            apresCB++;
    ...
    var wiki_url = wiki + "/w/api.php?action=query&prop=revisions&format=json&rvprop=timestamp|user&rvlimit=1&rvdir=newer&titles="+article;
  $.ajax({
      ...
      success: function (creation) {
          var data_c = creation.query.pages;
          var user_c;
          for(var key_c in data_c){
              user_c = data_c[key_c].revisions;
          }
          dateFirstContribution = user_c[0].timestamp;
          activeAjaxConnections--;
          
          var dateDiff_avant = getDayDifference(dateFirstContribution,dateContributionUser);
          var dateDiff_apres = getDayDifference(dateContributionUser,dateLastContribution);
          $("#suivie_intervention").html("<table><tr><td>Intervention</td><td title='A = Number of days between my intervention and the first one //  B = Number of interventions between my intervention and the first one'>A / B</td><td title='A = Number of days between my intervention and the last one // &nbsp; B = Number of interventions between my intervention and the last one'>A / B</td></tr><tr><td>My last intervention</td><td>&nbsp;"+dateDiff_avant+" days / "+overflow + avantCB+"&nbsp;</td><td>&nbsp;"+dateDiff_apres+" days / "+apresCB+"&nbsp;</td></tr></table>");
...
function getDayDifference(startDate, endDate) {
  ...
  var dayDifference = Math.ceil((end - start) / (1000 * 60 * 60 * 24));
  console.log("start:" + start + "end:" + end + "dayDifference:" + dayDifference);
...
function getIntervention(article){
  var wikiUrl;
    wikiUrl = wiki + "/w/api.php?action=query&prop=revisions&format=json&titles="+article+"&rvprop=timestamp|user&rvlimit=500";
  doGet(wikiUrl, "Q6");
}
